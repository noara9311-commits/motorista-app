...
function BloqueioScreen({onLogout,tipo}){
  ...
  return(
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#0f172a",minHeight:"100vh",maxWidth:430,margin:"0 auto",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center"}}>
      <div style={{fontSize:56,marginBottom:16}}>{isDemo?"🎮":"🔒"}</div>
      <div style={{fontSize:22,fontWeight:800,color:"#f8fafc",marginBottom:8}}>{isDemo?"Modo demo encerrado":"Período de teste encerrado"}</div>
      <div style={{fontSize:14,color:"#94a3b8",lineHeight:1.7,marginBottom:32}}>{isDemo?`Seus ${DEMO_DAYS} dias de demonstração terminaram. Crie uma conta grátis para 15 dias completos!`:"Seus 15 dias gratuitos terminaram. Assine para continuar com todos os seus dados."}</div>
      <div style={{background:"linear-gradient(135deg,#1e3a8a,#1e40af)",borderRadius:16,padding:"24px 20px",width:"100%",marginBottom:20}}>
        <div style={{fontSize:12,color:"#93c5fd",fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>{isDemo?"Trial Gratuito":"Plano Mensal"}</div>
        <div style={{fontSize:36,fontWeight:900,color:"#fff"}}>{isDemo?"GRÁTIS":"R$12,90"}</div>
        <div style={{fontSize:13,color:"#bfdbfe",marginTop:8}}>{isDemo?"15 dias completos grátis":"Cancele quando quiser"}</div>
      </div>
      <button style={{width:"100%",background:"linear-gradient(135deg,#2563eb,#1d4ed8)",border:"none",borderRadius:12,padding:15,color:"#fff",fontSize:16,fontWeight:700,cursor:"pointer",marginBottom:12}} onClick={()=>window.open(isDemo?"/":"https://seulink.kiwify.com.br","_self")}>{isDemo?"Criar conta grátis — 15 dias":"Assinar agora — R$12,90/mês"}</button>
      <button style={{background:"none",border:"none",color:"#475569",fontSize:13,cursor:"pointer"}} onClick={onLogout}>Usar outra conta</button>
    </div>
  );
}
...
export default function MotoristaApp(){
  ...
  return(
    <div style={S.root}>
      ...
      {acesso.status==="trial"&&acesso.daysLeft<=5&&<div style={{background:"linear-gradient(90deg,#92400e,#78350f)",padding:"8px 16px",fontSize:12,color:"#fcd34d",textAlign:"center",fontWeight:600}}>⚠️ Teste termina em {acesso.daysLeft} dia{acesso.daysLeft!==1?"s":""} · <span style={{textDecoration:"underline",cursor:"pointer"}} onClick={()=>window.open("https://seulink.kiwify.com.br","_blank")}>Assinar R$12,90/mês</span></div>}
      {acesso.status==="trial"&&acesso.daysLeft>5&&<div style={{background:"#1e293b",padding:"5px 16px",fontSize:11,color:"#64748b",textAlign:"center",borderBottom:"1px solid #334155"}}>🎁 {acesso.daysLeft} dias de teste restantes</div>}
      ...
      <div style={S.content}>
            ...
            return(<div key={dia} style={S.diaCard}>
              ...
              {ab&&cs.map(c=>(<div key={c.id} style={S.lancRow}>
                <div style={S.lancInfo}>
                  ...
                  {c.turno&&<span style={{background:"#1d4ed820",color:"#93c5fd",fontSize:10,padding:"2px 6px",borderRadius:99}}>{TURNOS.find(t=>t.id===c.turno)?.icon} {TURNOS.find(t=>t.id===c.turno)?.label}</span>}
        ...
        {tab===2&&<div style={S.page}>
          <div style={{background:"linear-gradient(135deg,#1e3a5f,#0f172a)",borderRadius:14,padding:"16px 14px",marginBottom:14,border:"1px solid #2563eb44"}}>
            <div style={{fontSize:11,fontWeight:700,color:"#60a5fa",textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>🧠 Radar Inteligente</div>
...
const S={
  ...
  cGreen:{background:"linear-gradient(135deg,#064e3b,#065f46)"},cBlue:{background:"linear-gradient(135deg,#1e3a8a,#1e40af)"},
